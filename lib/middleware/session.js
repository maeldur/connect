...
function session(options){
  options = options || {}
    ...
    , header = options.header || 'x-connect-sid'
    , headerFunc = options.headerFunc || null
  ...
  return function session(req, res, next) {
    ...
    var rawCookie = req.headers[header];
    var hasHeader = !!rawCookie;

    // prefer header if it exists
    if (!rawCookie) {
    ...
    res.on('header', function(){
      ...
      var cookieVal = cookie.serialize(key, val);
      debug('set-cookie %s', cookieVal);
      res.setHeader('Set-Cookie', cookieVal);
      if (headerFunc && headerFunc(req.session)) {
